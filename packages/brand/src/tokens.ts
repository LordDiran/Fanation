 ...
 *
 * Values are taken from the live landing page (`apps/landing`), which is the palette
 * the public has already seen. That decision was not arbitrary: 13 of the 16 rgba
 * tints already hand-written into `packages/ui/src/styles.css` were derived from
 ...
 *
 * `apps/landing` deliberately does NOT depend on this package. It keeps its own
 * inline hex, which already matches these values. Adding the dependency would make
 * every brand tweak rebuild the live marketing site, and that trade is not worth it.
 */
